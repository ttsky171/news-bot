...
# [추가] signal.bz 바로가기 버튼을 사이드바 최상단에 배치
...
def call_ai_prime_tech(key, sys_prompt, user_msg):
    ...
    try:
        ...
        if res.status != 200:
            return f"❌ 서버 응답 에러 (HTTP {res.status}): API 키나 충전 잔액을 확인하세요.\n상세 내용: {data.decode('utf-8', errors='ignore')}"
        
        result_json = json.loads(data.decode("utf-8"))
        
        # 겜스고/프록시 서버 응답에 맞춰 텍스트 추출 방식 강화
        content_list = result_json.get("content", [])
        if isinstance(content_list, list):
            output_text = "".join([b.get("text", "") for b in content_list if isinstance(b, dict) and b.get("type") == "text"])
        elif isinstance(content_list, dict): # 단일 객체로 올 경우 예외 처리
            output_text = content_list.get("text", "")
        else:
            output_text = str(content_list)
            
        if not output_text.strip():
            return "⚠️ API 응답은 성공했으나 반환된 텍스트 내용이 비어있습니다. 구조를 확인하세요."
...
if st.sidebar.button("✨ 플랫폼별 블로그 글 생성", type="primary"):
    if not api_key:
        ...
        # 데이터 유실을 방지하기 위해 결과를 먼저 딕셔너리에 수집합니다.
        results = {}
        ...
        # API 통신이 완료된 후 화면에 탭을 한 번에 그려 안전하게 출력합니다.
        st.success("🎉 모든 플랫폼 원고 생성이 완료되었습니다!")
        tabs = st.tabs(platforms)
        for idx, p in enumerate(platforms):
            with tabs[idx]:
                st.markdown(f"### 🖥️ {p} 최적화 포스팅 원고")
                # 결과 출력
                st.text_area(f"{p} 결과물 (드래그 복사 가능)", value=results.get(p, ""), height=500, key=f"text_{p}")
