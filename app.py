...
# 🔗 [수정완료] 진짜 시그널 실시간 트렌드 사이트로 주소 변경
    ...
    if st.button("🚀 1단계: 블로그 글 생성하기", type="primary"):
        if not api_key:
            ...
            for p in platforms:
                ...
                # 종합 프롬프트 조립 (시그널 사이트 맥락 인지 강화)
                sys_prompt = (
                    f"당신은 실시간 트렌드 전문 파워블로거이자 디지털 카피라이터입니다. '시그널 실시간 트렌드 사이트(signal.bz)'에서 수집된 타겟 키워드와, "
                    f"동기화되어 제공되는 실시간 이슈 요약본 데이터의 맥락을 완벽히 결합하여 대중의 관심사에 딱 맞는 깊이 있는 글을 작성해야 합니다.\n\n"
                    f"[톤앤매너 규칙]\n{tone_instruction}\n\n"
                    f"[플랫폼별 작성 규칙]\n{platform_instruction}\n\n"
                    ...
                    f"- 'AI 요약에 따르면', '시그널 웹사이트에 따르면' 같은 인위적인 문구는 절대 금지합니다. 직접 취재하고 분석한 트렌드 전문가처럼 작성하세요."
                )
                
                user_msg = (
                    f"● 실시간 트렌드 키워드 (출처: signal.bz): {query}\n"
                    f"● 시그널 동기화 팩트 요약 정보:\n{factual_summary}\n\n"
                    f"위 수집 데이터와 실시간 뉴스 맥락을 철저히 분석하여 대중들이 열광하고 유익해할 완성도 높은 포스팅을 작성해줘. "
