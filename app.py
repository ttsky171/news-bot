...
with tab1:
    # 수동 입력 모드일 때 메인 영역에 팩트 요약창 노출 (복구 완료!)
    ...
    if st.button("🚀 블로그 글 생성하기", type="primary"):
        if not api_key:
            ...
            for p in platforms:
                ...
                sys_prompt = (
                    f"당신은 각 블로그 플랫폼별 검색 노출 알고리즘을 완벽하게 이해하고 있는 파워블로거입니다. "
                    f"입력된 키워드와 뉴스 데이터를 분석하여 완벽한 상위 노출용 포스팅을 작성하세요.\n\n"
                    f"⚠️ [작성 안내]\n"
                    f"- 본문 맨 처음에 알맞은 제목을 크게 적어주고, 이어서 서론, 본문(소제목 포함), 결론을 한 흐름으로 작성해 주세요.\n"
                    f"- 이미지나 외부 링크 추천 관련 멘트는 절대로 적지 마세요.\n\n"
                    f"✨ [소제목 스타일 규칙 - 필수]\n"
                    f"- 본문 중간중간 내용을 구분 짓는 모든 소제목은 양옆에 '**'를 감싸서 무조건 굵은 글씨로 표현되게 하세요. (예시: **1. 핵심 내용 요약 및 분석**)\n"
                    ...
                    f"🏷️ [스마트블록/검색 최적화 태그 생성 규칙]\n"
                    f"- 결론이 완전히 끝난 뒤 가장 마지막 단락에, 뉴스 분류 및 타겟 노출에 최적화된 연관 핵심 해시태그들을 공백 구분 형태로 자연스럽게 20개 이상 나열해 주세요.\n"
                    f"- 복잡한 특수문자나 기호를 중첩하지 말고 순수하게 띄어쓰기와 '#' 기호로만 나열해야 합니다.\n\n"
                    f"[톤앤매너 규칙]\n{tone_instruction}\n\n"
                    f"[플랫폼별 작성 규칙]\n{platform_instruction}\n\n"
                    f"[⚠️ 절대 금지 규칙]\n"
